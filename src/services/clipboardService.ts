 ...
 * idSensor, dataRegistro, Foto_Topo, Foto_Base, Foto_Frente, Foto_Verso,
 * Foto_Esquerda, Foto_Direita, CaminhoPasta.
 */
export function buildSharePointRow(session: SensorSession): string {
  ...
  const cells = [session.idSensor, session.dataRegistro, ...fotoColumns, oneDriveFolderName()];
  return cells.join('\t');
}
